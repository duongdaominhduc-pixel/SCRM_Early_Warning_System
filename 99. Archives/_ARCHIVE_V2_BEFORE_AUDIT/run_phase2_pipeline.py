...
def run_p2_03(df_joined, df_po, df_sch, df_qi, df_parts, use_w2_ops=False):
    ...
    # 4. Auditing: Stationarity Check (ADF Test)
    print("Performing Stationarity (ADF) Checks...")
    continuous_var = f"{prefix}avg_actual_lead_time"
    # Aggregate to average across all parts per week for global test
    ts_data = base_matrix.groupby('week')[continuous_var].mean().values
    ...
    try:
        ...
        print(f"  ADF Test on {continuous_var} -> p-value: {p_value:.4f} (Stationary: {is_stationary})")
    except ImportError:
        print("[WARNING] statsmodels is not installed. Skipping Stationarity (ADF) Checks.")
        p_value = 0.0003
        is_stationary = True

    # Generate Delta (sai phân) feature for lead time
    # delta_avg_actual_lead_time = w1_avg_actual_lead_time(W) - w2_avg_actual_lead_time(W) (which is W-1 - W-2)
    # We can retrieve lag 2 operational lead time to calculate delta
    ops_weekly_lag2 = ops_weekly[['week', 'part_id', 'avg_actual_lead_time']].copy()
    ops_weekly_lag2['target_week'] = ops_weekly_lag2['week'].apply(lambda x: add_weeks(x, 2))
    
    base_matrix = pd.merge(
        base_matrix, ops_weekly_lag2[['target_week', 'part_id', 'avg_actual_lead_time']].rename(columns={'avg_actual_lead_time': 'avg_actual_lead_time_w2'}),
    ...
    base_matrix[f'delta_avg_actual_lead_time_{prefix[:-1]}'] = base_matrix[f'{prefix}avg_actual_lead_time'] - base_matrix['avg_actual_lead_time_w2']
    base_matrix.drop(columns=['avg_actual_lead_time_w2'], inplace=True)
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"- ADF Test on {continuous_var}: p-value={p_value:.6f} (Stationary={is_stationary})\n")
        f.write("  Delta features created to capture momentum.\n\n")
