...
# Helper: File Checksum (SHA256) for Data Governance
...
def run_p3_03(df_eval, optimal_thresholds):
    ...
    with open(integration_report_path, "w", encoding="utf-8") as f:
        ...
        f.write("## 3. Data Governance (SHA256 Checksums)\n")
        hero_chart_sha256 = calculate_sha256(hero_chart_path)
        f.write(f"- **Hero Chart SHA256:** `{hero_chart_sha256}`\n")

    report_sha256 = calculate_sha256(integration_report_path)
    print(f"Successfully saved Case Study Report to: {integration_report_path}")
    print(f"Integration Report SHA256: {report_sha256}")
